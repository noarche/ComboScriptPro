...
def extract_specific_text():
    ...
    text_to_find = input("Enter the text you want to extract: ")

    
    if not os.path.exists("extractedDomains"):
        os.makedirs("extractedDomains")

    matching_lines = [line for line in lines if text_to_find in line]
...
def extract_emails():
    ...
    emails = re.findall(pattern, ''.join(lines))
